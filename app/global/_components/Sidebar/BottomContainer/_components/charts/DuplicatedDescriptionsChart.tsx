...
function DuplicatedDescriptionsChart() {
  const {
    crawlData,
    javascript,
    css,
  ...
  const duplicatedDescriptions = issuesData?.length;

  const chartData = [
    // { browser: "HTML", visitors: totalPages, fill: "hsl(210, 100%, 50%)" },
    {
      browser: "Duplicated",
      visitors: duplicatedDescriptions || 0,
      fill: "hsl(710, 100%, 60%)",
    },
    {
      browser: "Normal",
      visitors: totalPages - duplicatedDescriptions || 0,
      fill: "hsl(210, 100%, 70%)",
    },
  ];
    ...
    <Card className="flex flex-col dark:bg-gray-900 bg-slate-100 border-0 shadow-none">
                  ...
                  if (viewBox && "cx" in viewBox && "cy" in viewBox) {
                    return (
                      ...
                      >
                        ...
                        >
                          {duplicatedDescriptions?.toLocaleString()}
                        </tspan>
                        <tspan
                          x={viewBox.cx}
                          y={(viewBox.cy || 0) + 24}
      ...
      <CardFooter className="flex-col gap-2 text-xs dark:text-white/50">
        <div className="leading-none text-muted-foreground">
          Found{" "}
          <span className="text-red-500">{duplicatedDescriptions || 0}</span>{" "}
          pages with duplicated descriptions.
        </div>
...
export default DuplicatedDescriptionsChart;
