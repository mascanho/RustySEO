...
function H2MissingChart() {
  ...
  const missingH2 = issuesData?.length;

  const chartData = [
    // { browser: "HTML", visitors: totalPages, fill: "hsl(210, 100%, 50%)" },
    {
      browser: "Missing",
      visitors: missingH2 || 0,
      fill: "hsl(710, 100%, 60%)",
    },
    {
      browser: "Contains",
      visitors: totalPages - missingH2 || 0,
      fill: "hsl(210, 100%, 70%)",
    },
  ];
    ...
    <Card className="flex flex-col dark:bg-gray-900 bg-slate-100 border-0 shadow-none">
      ...
      <CardFooter className="flex-col gap-2 text-xs dark:text-white/50">
        <div className="leading-none text-muted-foreground">
          Found <span className="text-red-500">{missingH2 || 0}</span> pages
