...
function LowContentChart() {
  ...
  const lowContent = issuesData?.length;

  const chartData = [
    // { browser: "HTML", visitors: totalPages, fill: "hsl(210, 100%, 50%)" },
    {
      browser: "Too small",
      visitors: lowContent || 0,
      fill: "hsl(710, 100%, 60%)",
    },
    {
      browser: "Normal",
      visitors: totalPages - lowContent || 0,
      fill: "hsl(210, 100%, 70%)",
    },
  ];
    ...
    <Card className="flex flex-col dark:bg-gray-900 bg-slate-100 border-0 shadow-none">
      ...
      <CardFooter className="flex-col gap-2 text-xs dark:text-white/50">
        <div className="leading-none text-muted-foreground">
          Found <span className="text-red-500">{lowContent || 0}</span> pages
          with low content
        </div>
...
export default LowContentChart;
