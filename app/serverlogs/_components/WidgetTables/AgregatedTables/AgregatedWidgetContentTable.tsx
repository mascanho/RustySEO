...
interface AgregatedWidgetContentTableProps {
  type: "status" | "method" | "useragent" | "referer" | "browser" | "verified" | "ip" | "path_analysis";
  title: string;
  segment: string;
}
...
const getStatusCodeInfo = (statusCode: number) => {
  if (statusCode >= 200 && statusCode < 300) {
    return { textColor: "text-green-800 dark:text-green-200", bgColor: "bg-green-100 dark:bg-green-900" };
  } else if (statusCode >= 300 && statusCode < 400) {
    return { textColor: "text-yellow-800 dark:text-yellow-200", bgColor: "bg-yellow-100 dark:bg-yellow-900" };
  } else if (statusCode >= 400 && statusCode < 500) {
    return { textColor: "text-orange-800 dark:text-orange-200", bgColor: "bg-orange-100 dark:bg-orange-900" };
  } else if (statusCode >= 500 && statusCode < 600) {
    return { textColor: "text-red-800 dark:text-red-200", bgColor: "bg-red-100 dark:bg-red-900" };
  }
  return { textColor: "text-gray-800 dark:text-gray-200", bgColor: "bg-gray-100 dark:bg-gray-900" };
};
...
export const AgregatedWidgetContentTable: React.FC<AgregatedWidgetContentTableProps> = ({
  ...
  const getColumns = () => {
    ...
    let specific = [];
    switch (type) {
      case "status": specific = [{ key: "status", label: "Status Code", icon: <Hash className="w-3 h-3" /> }]; break;
      ...
      case "verified": specific = [
        { key: "crawler_type", label: "Crawler", icon: <Bot className="w-3 h-3" /> },
      ...
      case "path_analysis": specific = [{ key: "crawler_type", label: "Crawler", icon: <Bot className="w-3 h-3" /> }]; break;
    }
  ...
  const renderValue = (item: any, key: string) => {
    ...
    if (key === "verified") {
      return val ? (
        ...
        <Badge variant="outline" className="bg-red-600 text-white dark:bg-red-400 border-purple-200 gap-1 text-[10px]">
    ...
    if (key === "status") {
      ...
      return (
        <Badge variant="outline" className={`${info.bgColor} ${info.textColor} font-mono text-[10px]`}>
          {val}
        </Badge>
      );
    }
    ...
    return <span className="truncate max-w-[250px] block dark:text-white/70 text-xs" title={val}>{val || "-"}</span>;
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {/* FOOTER / PAGINATION */}
      <div className="flex items-center justify-between w-full px-1" style={{ marginTop: "0.2em" }}>
        <div className="flex items-center -mt-2 ml-1 gap-4">
            ...
            >
              ...
              <SelectContent className="z-[9999999999] dark:bg-brand-darker dark:border-brand-dark">
                <SelectItem className="dark:hover:bg-brand-bright dark:hover:text-white hover:bg-brand-bright hover:text-white" value="50">50</SelectItem>
                <SelectItem className="dark:hover:bg-brand-bright dark:hover:text-white hover:bg-brand-bright hover:text-white" value="100">100</SelectItem>
                <SelectItem className="dark:hover:bg-brand-bright dark:hover:text-white hover:bg-brand-bright hover:text-white" value="500">500</SelectItem>
                <SelectItem className="dark:hover:bg-brand-bright dark:hover:text-white hover:bg-brand-bright hover:text-white" value="1000">1000</SelectItem>
              </SelectContent>
            </Select>
            <div className="text-xs text-slate-500 dark:text-white/50">
              Showing <span className="font-bold text-slate-900 dark:text-white">{((page - 1) * itemsPerPage) + 1}</span> to <span className="font-bold text-slate-900 dark:text-white">{Math.min(page * itemsPerPage, totalCount)}</span> of <span className="font-bold text-slate-900 dark:text-white">{totalCount.toLocaleString()}</span> entries
            </div>
        </div>

        <Pagination className="text-xs w-auto mx-0">
          <PaginationContent style={{ marginTop: "-5px" }}>
            ...
            <div className="flex items-center gap-1 mx-2">
                ...
                <Badge variant="outline" className="h-6 min-w-[24px] flex justify-center dark:border-brand-dark dark:text-white font-mono">
                    {page}
                </Badge>
                <span className="text-xs dark:text-white/50">of {totalPages || 1}</span>
            </div>
